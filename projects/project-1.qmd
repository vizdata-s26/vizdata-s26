...
You can choose any dataset you want from the datasets released in 2025 as part of this project: <https://github.com/rfordatascience/tidytuesday/blob/master/data/2025/readme.md>.
...
Each of the two questions you come up with should involve more than two variables to answer.
...
-   Week 1 of project (week of January 26): Choose a dataset and write up your proposal in `proposal.qmd`.

-   Week 2 of project (week of February 2): Provide peer review of one other team's proposal in the form of issues in their GitHub repos.
    Start addressing the issues left on your team's project repo by closing them with explicit commits.
    Feel free to get started on your presentation as well.

-   Week 3 of project (week of February 9): Finish addressing the issues left on your team's project repo by closing them with explicit commits and submit your proposal for instructor review.
    Later in the week review feedback from me on your proposal and close any remaining issues.
    Work on your presentation in `presentation.qmd` and write-up in `index.qmd`.

-   Week 4 of project (week of February 16): Continue to work on your presentation in `presentation.qmd` and write-up in `index.qmd`.

-   Week 5 of project (week of February 23): Finalize your write-up, presentation, and your project website.
    Present in lab on the Thursday of this week.

## Due dates

-   Proposals for peer review: due on Wednesday, February 4 at 1 pm.
-   Revised proposals for teaching team review: due Wednesday, February 11 at 1 pm.
-   Write-up and presentation: due Thursday, February 26 at the **beginning** of your lab section.

## Workflow

- Clone your `project-1-team_name` repo from the course GitHub organization.
- Update `_quarto.yml`:
    - `title` field: Title of your project
    - `author` field: Your team name
- Type your proposal in `proposal.qmd`.
- Add your data to `data/` and update the `README.md` file in that folder with the codebook for your dataset(s).
- Preview your proposal by clicking on the Preview button in Positron while in `proposal.qmd`.
- Preview your entire project by going to the Terminal in Positron and typing `quarto preview`.
- Commit and push your changes to your GitHub repo.
- Make sure the GitHub action that renders your project passes. If it doesn't, review the errors, implement the fix, and re-commit and re-push.
- Preview your deployed project at <https://vizdata-s26.github.io/project-1-team_name> (this URL is also in your repo).
- (Optional) Update `_brand.yml` if you want different colors, fonts, theming, etc. Learn more [here](https://quarto.org/docs/authoring/brand.html).
...
Each team will review the proposal of one other team.
The peer review will be completed during a lab.
On the peer review day, teams will have access to the project repos of the team whose work they're reviewing.
Reviews should start by cloning the team's repo, re-rendering it locally to make sure you can reproduce their work, and then adding an issue to their repo with your peer review feedback.

The reviewer / reviewee assignments are posted below:
...
gt(p1_pr) |>
  cols_label(
    section = "Section",
    reviewer_original = "Reviewer",
    reviewee_original = "Reviewee"
  )
```
...
2.  **Question 1**: The title should relate to the question you're answering.
    ...
    -   Analysis (2 figures, text/code comments as needed): In this section, provide the code that generates your plots.
...
Your presentation will be created using Quarto, which allows you to write slides using the same Quarto structure you're used to.
There is a sample Quarto slide template in your repo you can edit to your heart's desire to create your presentation.
...
Each of your projects will have a website that looks like <https://vizdata-s26.github.io/project-1> (to be posted).
You are not expected to change the styling of the website, but if you want to, you'll need to edit the `_quarto.yml` file in your repo.
...
-   `_brand.yml`: Setup colors, fonts, and theming for project website
...
[^5]: Code style and smell does count for this assignment, so please take the time to make sure everything looks good and your data and code are properly formatted.
...
[^7]: This relates to the quality and quantity of the peer review you provide for another team.
...
### Reproducibility and organization (5 points)

-   All required files are provided. Quarto files render without issues and reproduce the necessary outputs. (2 points)
-   Data is in the data folder, with a codebook in the README, and is loaded from this folder in presentation and writeup. (1 points)
-   Documents are well structured and easy to follow. No extraneous materials. (1 points)
-   All issues are closed, mostly with specific commits addressing them. (1 points)

### Code style and smell (10 points)

-   Consistent naming conventions: Variable names, functions, and data objects use clear, descriptive snake_case naming. Names reflect their purpose in the visualization (e.g., sales_by_region, not df1 or temp). (2 points)
-   Readable formatting & spacing: Code follows tidyverse formatting conventions: consistent indentation, spaces around operators, line breaks for long pipelines, and one statement per line where appropriate. (2 points)
-   Effective Use of the base pipe (\|\>): Pipes are used to create clear, linear data transformation and visualization workflows. Pipelines are not overly long or deeply nested, and intermediate steps are understandable. (1 points)
-   Avoidance of redundant or dead code: No unused variables, commented-out blocks, duplicated code, or unnecessary intermediate objects that suggest poor cleanup or experimentation left in the final submission. Where applicable, repeated logic (e.g., data cleaning or plotting patterns) is factored into functions that are short, focused, and named clearly, rather than copy-pasted. (2 points)
-   Clear and purposeful comments: Comments explain why a step exists rather than restating what the code does. Comments are used sparingly and are up to date with the code. (1 point)
-   Avoidance of common code smells: Code avoids magic numbers, overly complex expressions, hard-coded paths, and deeply nested conditionals that reduce readability and flexibility. (2 points)

## Guidelines

Please use the project repository that has been created for your team to complete your project.
This means putting all of the content in the Quarto files provided, rendering them to obtain the output, and committing and pushing all files to your repository by the indicated deadlines.
Your Quarto files (`.qmd`) and the resulting html files (`.html`) will be graded jointly, so they must be consistent (as in, don't change the Quarto file without also updating the rendered document!).
...
-   Code:
    ...
    -   In your write-up and your presentation your code should be hidden (`echo: false`) so that your slides are neat and easy to read and your write-up is about your narrative and results only.
    ...
    -   Even though your code is hidden, it should still confirm to good coding practice, style, etc.
        You will be evaluated on code quality as well, which we will check by explicitly reviewing your Quarto documents (not just the rendered output).
